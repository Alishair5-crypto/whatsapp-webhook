...
test('complete catalogue gets 50 product limit', () => assert.deepEqual(extractFilters('complete catalogue'), { fabric: '', color: '', collection: '', name: '', limit: 50 }));
...
test('catalogue context never claims photos when records have no images', () => {
  const context = buildContext([{ name: 'Test Lawn', collection: 'Printed', fabric: 'Lawn', color: 'Blue', price: 5000, currency: 'PKR', stock_quantity: 2, inventory_verified: true, images: [] }], { fabric: 'Lawn', color: 'Blue', collection: '', name: '', limit: 5 });
