...
function getSql(dbUrl) {
  ...
  try {
    ...
    console.error('[CATALOGUE DB INIT]', e.message);
    return null;
  }
}
...
function getBlob() {
  try {
    ...
    console.error('[CATALOGUE BLOB INIT]', e.message);
    return null;
  }
}
...
function validateImageInput({ productId, buffer, contentType, altText = '', sortOrder = 0 }) {
  ...
  if (!ALLOWED_TYPES[normalizedType]) throw new Error('Unsupported image type');
  if (detectImageType(buffer) !== normalizedType) throw new Error('Image content does not match type');
  ...
  if (!Number.isInteger(normalizedSortOrder) || normalizedSortOrder < 0 || normalizedSortOrder > 10000) {
    throw new Error('Invalid image sort order');
  }
  ...
  return {
    ...
    extension: ALLOWED_TYPES[normalizedType],
    altText: cleanText(altText),
    sortOrder: normalizedSortOrder
  };
}

function makeBlobPath(productId, extension) {
...
async function uploadProductImage({
  ...
  try {
    uploaded = await blob.put(pathname, input.buffer, {
      access: 'public',
...
async function listProductImages(dbUrl, productId) {
