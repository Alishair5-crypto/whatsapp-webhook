...
//   BASE: Fully Audited Production Version (Safe Response Wrapper + Neon DB)
// ─────────────────────────────────────────────────────────────────────────────
const crypto = require('crypto');

// ── Bulletproof Response Helper (Fixes Vercel res.status crash) ───────────────
function sendResponse(res, code, data = '') {
  try {
    if (typeof res.status === 'function') {
      if (data === null || data === undefined) return res.status(code).end();
...
module.exports = async (req, res) => {
  try {
    if (req.url?.includes('favicon.ico')) return sendResponse(res, 204);
    midnightReset();
    ...
    // ── Vercel Cron Job Handler for Re-engagement ─────────────────────────────
    const isCron = url.searchParams.get('cron') === 'true' || req.headers['x-vercel-cron'];
    if (isCron) {
      const cronSecret = 'fatima_arts_secure_cron_123';
      if (url.searchParams.get('secret') !== cronSecret) {
        return sendResponse(res, 401, 'Unauthorized Cron');
      }

      const sql = getSql(DATABASE_URL);
      if (sql && WHATSAPP_TOKEN && PHONE_NUMBER_ID) {
        try {
          const inactiveRows = await sql`
            SELECT phone_number, customer_name, history, last_seen 
          ...
          for (const row of inactiveRows) {
            ...
            if (history.length > 0) {
              ...
              const alreadyFollowedUp = history.some(h => h.parts?.[0]?.text?.includes('[follow-up-sent]'));
              
              if (lastMsg.role === 'model' && !alreadyFollowedUp) {
                const custName = row.customer_name ? ` ${row.customer_name}` : '';
                const followUpText = `Ji${custName} sahib/ji! Aapka suit finalize hona rehta hai, koi confusion toh nahi size ya fabric mein? Bataiye taake order book kar dein 😊`;

                await fetch(`https://graph.facebook.com/v20.0/${PHONE_NUMBER_ID}/messages`, {
                  method: 'POST',
                  headers: { 'Authorization': `Bearer ${WHATSAPP_TOKEN}`, 'Content-Type': 'application/json' },
                  body: JSON.stringify({ messaging_product: 'whatsapp', to: row.phone_number, type: 'text', text: { body: followUpText } })
                });

                history.push({ role: 'model', parts: [{ text: followUpText + ' [follow-up-sent]' }] });
                await dbSave(DATABASE_URL, row.phone_number, row.customer_name, history);
              }
            }
          }
        } catch (cronErr) {}
      }
      return sendResponse(res, 200, 'Cron executed successfully');
    }
...
=== ORDER & MISSING DETAIL TRACKER (CRITICAL) ===
- **Memory & Context Audit:** Before replying, carefully scan the entire chat history. Check which details have ALREADY been provided by the customer and what is still MISSING.
- **Never Repeat Questions:** If the customer has already mentioned their product, size, address, city, or payment method, **NEVER ask for it again**.
- **Step-by-Step Missing Flow:** Collect *only* what is missing one by one smoothly:
  1. Product & Color confirmation (agar nahi bataya)
  2. Quantity & Size / Unstitched (agar nahi bataya)
  3. Full Delivery Address & City (agar nahi bataya)
  4. Payment Method - COD / JazzCash / EasyPaisa (agar nahi bataya)
- **Order Tag Generation:** Only generate the \`[ORDER:...]\` tag when ALL 4 requirements (Product, Qty/Size, Address+City, Payment) are 100% collected.
...
[ORDER:orderId=FA-584920|name=CustomerName|product=Product|qty=1|size=Unstitched|price=3600|payment=COD|address=Full Address|city=Faisalabad]`;
    ...
    if (req.method === 'GET') {
      ...
      if (mode && token) {
        if (mode === 'subscribe' && String(token).trim() === String(VERIFY_TOKEN).trim()) {
          return sendResponse(res, 200, challenge);
        }
        return sendResponse(res, 403, 'Verification Token Mismatch');
      }
      return sendResponse(res, 200, 'Webhook Endpoint Active');
    }
    ...
    if (req.method === 'POST') {
      ...
      if (!messages.length) return sendResponse(res, 200, 'EVENT_RECEIVED');
      if (!WHATSAPP_TOKEN || !PHONE_NUMBER_ID) return sendResponse(res, 200, 'EVENT_RECEIVED');

      const message = messages[0];
      if (!message) return sendResponse(res, 200, 'EVENT_RECEIVED');

      const msgId = message?.id;
      if (alreadyProcessed(msgId)) return sendResponse(res, 200, 'EVENT_RECEIVED');

      const fromNumber = message.from;
      if (!fromNumber) return sendResponse(res, 200, 'EVENT_RECEIVED');
    ...
    console.error('[FATAL HANDLER CRASH]', fatalErr);
    return sendResponse(res, 200, 'EVENT_RECEIVED');
  }
};
