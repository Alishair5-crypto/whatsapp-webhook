...
module.exports=async(req,res)=>{
 ...
 if(!dbUrl||!dbUrl.startsWith('postgres')) return res.status(200).json({ok:true,available:false,leads:[],conversations:[],products:[]});
 ...
 try{
  ...
  let products=[];
  if(scope==='products'){
   const rows=await sql`SELECT p.id,p.name,p.collection,p.fabric,p.color,p.price,p.currency,p.description,i.stock_quantity,pi.image_url,pi.alt_text,pi.is_primary
    FROM catalog_products p LEFT JOIN catalog_inventory i ON i.product_id=p.id
    LEFT JOIN LATERAL (SELECT ci.image_url,ci.alt_text,ci.is_primary FROM catalog_images ci WHERE ci.product_id=p.id ORDER BY ci.is_primary DESC,ci.sort_order ASC,ci.id ASC LIMIT 1) pi ON true
    WHERE p.status='active' ORDER BY p.updated_at DESC,p.id DESC LIMIT 80`;
  ...
  return res.status(200).json({ok:true,available:true,leads,conversations,products});
 }catch(e){console.error('[DASHBOARD LIVE]',e?.message||e);return res.status(200).json({ok:true,available:false,leads:[],conversations:[],products:[],error:'Live dashboard data unavailable'});}
};
